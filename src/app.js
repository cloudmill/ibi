...
// Добавление активного состояния на автора в публикации
{
  $(() => {
    $(window).on("click", (event) => {
      const authorTag = $(event.target).closest(
        ".articles-authors__filter-item"
      );
  ...
  if (!mediaQuery.matches) {
    try {
      function seqMargin() {
        ...
        const heightSlide = $(
          ".seq__texts--mobile .seq__texts-item--last"
        )[0].getBoundingClientRect().height;
        const mainContent = $(".implantation-main-content");

        const margin = -(heightSlide - heightText + 60);

        setTimeout(() => sendSignal("mobile-seq:margin", margin));
    ...
    window.addEventListener("click", (e) => {
      ...
      if (trigger.length) {
        ...
        $("html, body").animate(
          {
            scrollTop:
              $(target).offset().top - (mediaQuery.matches ? 140 : 100),
          },
          500
        );
