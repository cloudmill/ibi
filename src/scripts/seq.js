...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (seq) {
    ...
    // для десктопа, планшета: определяем высоту подложки с учетом размеров области, в которой можно расположить подложку, соотношения длины <canvas> к длине текста и соотношения сторон изображений
    // на мобиле ситуация проще: там квадратный <canvas>
    if (getMediaQuery(BREAKPOINT.DEFAULT).matches) {
      /*
        h - высота области в которой может располагаться подложка (на которой расположены <canvas> и тексты)
        w - длина области в которой может располагаться <canvas> с учетом соотношения (1245 / 1840)
        (1245 / 1840) - отношение длины изображения к длине всей подложки, взято из макета (десктоп)
        ratio - отношение длины к высоте области (w * h) в которой может располагаться <canvas> 
      */
      const h =
